...
<%
	if (BaseServlet.isAdmin(request)) {
		...
		out.println("  <li  class=\"path\"><a href=\"repository_import.jsp\">存储导入</a></li>");
		out.println("</ul>");
		out.println("<br/>");
		...
		out.println("<td><a class=\"ds\" href=\"../extension/DataBrowser?action=fs&sel=fld&dst=fsPath&root="+Config.INSTANCE_CHROOT_PATH+"\"><img src=\"img/action/browse_fs.png\"/></a></td>");
		out.println("</tr>");
		out.println("<tr>");
		out.println("<td>存储路径</td>");
		out.println("<td colspan=\"2\"><input type=\"text\" size=\"50\" name=\"repoPath\" id=\"repoPath\" value=\""+repoPath+"\" ></td>");
		out.println("<td><a class=\"ds\" href=\"../extension/DataBrowser?action=repo&sel=fld&dst=repoPath\"><img src=\"img/action/browse_repo.png\"/></a></td>");
		out.println("</tr>");
		out.println("<tr>");
		out.println("<td>源数据</td>");
		out.println("<td colspan=\"2\"><input type=\"checkbox\" name=\"metadata\" "+(metadata?"checked":"")+"/></td>");
		out.println("</tr>");
		out.println("<tr>");
		out.println("<td>历史</td>");
		out.println("<td colspan=\"2\"><input type=\"checkbox\" name=\"history\" "+(history?"checked":"")+"/></td>");
		out.println("</tr>");
		out.println("<tr>");
		out.println("<td>存储  UUIDs</td>");
		out.println("<td width=\"23px\"><input type=\"checkbox\" name=\"uuid\" "+(uuid?"checked":"")+"/></td>");
		out.println("<td><span style=\"color: red;\">(请谨慎使用)</span></td>");
		out.println("</tr>");
